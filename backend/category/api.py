...
def create_category(category: RegisterCategory = Body(...)) -> Any:
    """
    Register Category API
    """
    inserted_id = insert_item(category_collection, category)
    response_data = success_response(data={'item_id': str(inserted_id)}, message="Successfully inserted data")
    return JSONResponse(content=response_data, status_code=201)


@router.get("/v1/category", response_description="Get Category List")
def get_category(
        page_number: int = 1,
        page_size: int = 10,
